...
- Apply the `question` label if it exists in the repository.
...
- Prefer this repository's standard labels when supported by the issue content: `bug`, `enhancement`, `documentation`, `security`, `testing`, `accessibility`, `dx`, `performance`, `duplicate`, `invalid`, `question`, `help wanted`, `good first issue`, `backlog`, `dependencies`, `javascript`, and `github_actions`.
- This repository does not currently have explicit severity or priority labels. Do not invent or apply severity/priority labels that are not present.
- If explicit severity or priority labels are added to the repository later, apply them only when impact and urgency are clear from the issue content.
...
Assess maintainer-facing severity for the triage comment, even when no severity label exists:
- **High**: security issue, data loss, broken core workflow, installation blocked for many users, or clear widespread regression.
- **Medium**: important feature area impaired, workaround exists, or issue affects a subset of users.
- **Low**: minor UI issue, documentation gap, small enhancement, polish, or low-impact maintenance.
- **Unknown**: impact cannot be determined from the issue content.

Only use a severity other than **Unknown** when the issue content supports it. Do not infer production impact, affected versions, platforms, or user count unless stated or directly evidenced.
...
| **Severity** | [High / Medium / Low / Unknown] | [brief] |
